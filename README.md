...
- Editing works from a source-only RPG Maker MV/MZ project. The top-bar Play action first uses a complete runtime beside the project. Source-only MZ projects can borrow `nwjs-win/nw.exe` from a local RPG Maker MZ installation; source-only MV projects use the original editor playtest runner at `nwjs-win-test/Game.exe`. Right-click Play to inspect or change the current engine's runtime. The app remembers each engine separately, never copies the runtime, searches arbitrary game folders, or downloads a runner. Isolated Battle Test keeps its stricter project-local runtime boundary.
