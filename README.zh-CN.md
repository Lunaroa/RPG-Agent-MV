...
- MZ 1.10.0 仍是完整试玩验证基线；其他可识别的 MZ 核心版本允许尝试普通试玩，但运行成功不代表已经获得完整兼容保证。
- 工程标记不是必需文件。系统会自动识别核心脚本、数据布局、图块尺寸、画布尺寸和加密状态；加密工程可在警告后加载，但加密素材的列表、预览、替换和资源校验可能受限。MV/MZ 文件混用、数据布局冲突、必要数据缺失或核心版本无法识别时仍会拒绝。
- 编辑只需要源工程。普通试玩优先使用工程自身运行器；没有 `Game.exe` 的 MZ 工程可以选择一次本机 MZ 安装目录中的 `nwjs-win\nw.exe`，MV 工程会优先使用标准安装目录中的 `nwjs-win\Game.exe`。两种运行器分开保存、原地使用，不会被复制进游戏工程；隔离战斗测试仍保留更严格的工程本地运行器边界。
