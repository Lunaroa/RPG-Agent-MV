...
- **不会**自动看到 RPG-Agent-MV-MV 桌面端的实时界面；没有截屏、浏览器或 computer-use 类工具（见 `config/agents/default/agent.yaml` 与 `config/capabilities/tool-manifest.json` 白名单）。
...
- 新增事件不得直接替用户决定坐标或落点；默认先生成 / 注册为待确认事件草稿，用户确认后才进入待放置队列，由用户在 **RPG-Agent-MV-MV 桌面端地图画布**上拖放确认后再写入 Map JSON（**不是**外部 RPG Maker MV 编辑器）。
...
3. **人工放置**：用户在 **桌面端地图画布**（RPG-Agent-MV-MV 内置地图视图，不是外部 RMMV 编辑器）自行切换目标地图，将待放置事件拖放到合适位置；系统以**点击时当前打开的地图**写入 Map JSON 并回写注册表。注册时的 `mapId` / `targetMapId` 仅为 Agent 建议，不限制落点地图。
