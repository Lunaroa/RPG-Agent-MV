...
The global **Play / Stop** control launches the current source project. MV and MZ both use their project-local runtime; MZ statically validates `Game.exe` and its adjacent NW.js files and does not require the editor to be installed. Interactive playtesting excludes staged changes, and the game may write to its own `save/` directory.
