...
The global **Play** action runs only the source project from the configured `System.json` start location and excludes staging. It checks the project-local runtime first. A source-only MZ project uses the saved `nwjs-win/nw.exe`; a source-only MV project first tries the standard installed `nwjs-win/Game.exe` and asks for it only when necessary. The runtime stays in its installation directory. MZ 1.10.0 remains the fully validated baseline; another recognizable MZ version is only allowed to attempt a normal playtest launch. The app does not scan arbitrary game folders or download a runner. The game may write to `save/`. When staging exists, its summary must be confirmed again whenever it changes.
