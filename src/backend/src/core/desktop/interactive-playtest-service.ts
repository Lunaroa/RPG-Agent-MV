...
export class InteractivePlaytestService {
  ...
  async start(
    ...
    return this.#start(projectRoot, options);
  }
  ...
  async #start(
    ...
    const run: InteractivePlaytestRun = {
      ...
      logPath: artifactLocations.logPath,
      stdoutPath: artifactLocations.stdoutPath,
      stderrPath: artifactLocations.stderrPath,
    };
    initializeArtifacts(run, artifactLocations);
    this.#currentRun = run;
    ...
    return new Promise<InteractivePlaytestResult>((resolve) => {
      ...
      const stdoutCapture = { flush: attachOutput(child.stdout, artifactLocations.stdoutPath, artifactLocations.logPath, outputRuntimeExecutable) };
      const stderrCapture = { flush: attachOutput(child.stderr, artifactLocations.stderrPath, artifactLocations.logPath, outputRuntimeExecutable) };
      const flushOutput = () => {
        stdoutCapture.flush();
        stderrCapture.flush();
      };
  ...
  #publish(run: InteractivePlaytestRun, logMessage: string): void {
    ...
    writeJsonAtomic(artifacts.artifactPath, run);
