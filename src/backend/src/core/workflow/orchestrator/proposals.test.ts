...
test("proposeWorkflow 落盘 pending：脚本存成独立 .js 文件，JSON 只存路径", () => {
  ...
  const proposal = proposeWorkflow(
    { workflowRoot: root, project: projectPath(root), script: SCRIPT, summary: "审 示例角色 的台词", title: "写作审核", sessionId: "s1", productLanguage: "zh-CN" },
    { makeId: seqId, now: fixedNow },
  );
  assert.equal(proposal.status, "pending");
  assert.equal(proposal.title, "写作审核");
  assert.equal(proposal.summary, "审 示例角色 的台词");
  assert.equal(proposal.project, projectPath(root));
  assert.equal(proposal.sessionId, "s1");
  assert.equal(proposal.productLanguage, "zh-CN");
...
test("readProposalScript：脚本文件缺失时 fail fast", () => {
  const root = tmpRoot();
  assert.throws(() => readProposalScript(root, "wp-missing", "zh-CN"), /脚本文件缺失/);
});
...
test("proposeWorkflow 标题/说明缺省回落，不报错", () => {
  const root = tmpRoot();
  const englishProposal = proposeWorkflow(
    { workflowRoot: root, project: projectPath(root), script: SCRIPT },
    { makeId: seqId, now: fixedNow },
  );
  assert.equal(englishProposal.title, "Workflow script");
  assert.match(englishProposal.summary, /read-only/);
  assert.equal(englishProposal.productLanguage, "en-US");

  const chineseProposal = proposeWorkflow(
    { workflowRoot: root, project: projectPath(root), script: SCRIPT, productLanguage: "zh-CN" },
    { makeId: seqId, now: fixedNow },
  );
  assert.equal(chineseProposal.title, "工作流脚本");
  assert.match(chineseProposal.summary, /只读/);
  assert.equal(chineseProposal.productLanguage, "zh-CN");
});
...
test("proposeWorkflow 空脚本直接抛、不留半截提议", () => {
  ...
  assert.throws(
    () => proposeWorkflow(
      { workflowRoot: root, project: projectPath(root), script: "   ", productLanguage: "zh-CN" },
      { makeId: seqId, now: fixedNow },
    ),
    /非空的编排脚本/,
  );
...
test("approveProposal：不能重复批准已决提议", async () => {
  const root = tmpRoot();
  const proposal = proposeWorkflow({ workflowRoot: root, project: projectPath(root), script: SCRIPT, productLanguage: "zh-CN" }, { makeId: seqId, now: fixedNow });
...
test("proposalId 路径穿越被拒绝（readProposal/readProposalScript/approve/reject）", () => {
  ...
  for (const evil of ["../../etc/passwd", "..\\..\\secret", "wp-..-x", "wp-/etc/passwd", "good.js"]) {
    assert.throws(() => readProposal(root, evil), /Invalid proposalId/, `readProposal should reject ${evil}`);
    assert.throws(() => readProposalScript(root, evil), /Invalid proposalId/, `readProposalScript should reject ${evil}`);
    assert.throws(() => rejectProposal(root, evil), /Invalid proposalId/, `rejectProposal should reject ${evil}`);
  }
});
...
test("approveProposal 并发双重批准被锁挡住（不重复执行）", async () => {
  ...
  await assert.rejects(
    ...
    /EEXIST|already exists|cannot|being handled|非法|状态|锁占用|正在被另一个进程处理/,
