...
const TEST_WORKFLOW_ROOT = path.join(os.tmpdir(), "workflow-runtime-test");
const TEST_PROJECT = path.join(TEST_WORKFLOW_ROOT, "project");
...
function fakeCtx(overrides: Partial<WorkflowContext> = {}): { ctx: WorkflowContext; logs: string[] } {
  ...
  const ctx: WorkflowContext = {
    ...
    abort: () => {},
    args: { role: "示例角色" },
    workflowRoot: TEST_WORKFLOW_ROOT,
...
test("空脚本 / 纯空白 → fail fast", async () => {
  const { ctx } = fakeCtx();
  await assert.rejects(() => runScriptInSandbox("", ctx, { productLanguage: "zh-CN" }), /编排脚本为空/);
  await assert.rejects(() => runScriptInSandbox("   \n  ", ctx), /orchestration script is empty/i);
});
...
test("无 await 的同步死循环被同步段超时护栏截断", async () => {
  ...
  await assert.rejects(
    () => runScriptInSandbox("while (true) {}", ctx, { syncTimeoutMs: 50, productLanguage: "zh-CN" }),
    /编排脚本编译\/启动失败/,
  );
});
    ...
    () =>
      runScriptInSandbox(
        `await agent({ prompt: "x", label: "y" });
         while (true) {}`,
        ctx,
        { scriptTimeoutMs: 200, productLanguage: "zh-CN" },
      ),
    /编排脚本执行超时/,
  );
...
test("await Promise.resolve 后死循环同样在短超时内被 terminate", async () => {
  ...
  await assert.rejects(
    () =>
      runScriptInSandbox(`await Promise.resolve(); while (true) {}`, ctx, { scriptTimeoutMs: 200, productLanguage: "zh-CN" }),
    /编排脚本执行超时/,
  );
  const elapsed = Date.now() - started;
  assert.ok(elapsed < 3000, `must terminate quickly, took ${elapsed}ms`);
...
test("脚本编译错误被包装成清晰错误", async () => {
  const { ctx } = fakeCtx();
  await assert.rejects(
    () => runScriptInSandbox("return (", ctx, { productLanguage: "zh-CN" }),
    /编排脚本编译\/启动失败/,
  );
});
...
test("buildScriptModule：标题/说明回落默认，run 跑脚本", async () => {
  ...
  assert.match(empty.description, /read-only/);

  const chinese = buildScriptModule({ script: "return 1;", productLanguage: "zh-CN" });
  assert.match(chinese.description, /只读/);
