...
function autotileShapeForKind(kind: number, neighbours: NeighbourConnectivity): number {
  ...
  if (classification.strategy === "floor") return floorAutotileShape(neighbours);
  if (classification.strategy === "wall") return wallAutotileShape(neighbours);
  if (classification.strategy === "waterfall") return waterfallAutotileShape(neighbours);
  throw new Error(`Autotile kind ${kind} is not supported: ${classification.reason || classification.strategy}`);
}

function autotileIdForKindAndNeighbours(kind: number, neighbours: NeighbourConnectivity): number {
  return makeAutotileId(kind, autotileShapeForKind(kind, neighbours));
}
...
function resolveAutotileLayer(kindGrid: Int32Array | number[], width: number, height: number): number[] {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      out[y * width + x] = autotileIdForKindAndNeighbours(kind, nb);
    }
  }
  return out;
}
