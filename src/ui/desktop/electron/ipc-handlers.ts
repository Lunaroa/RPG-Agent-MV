...
export async function initializeIpcHandlers(roots: AppRoots): Promise<void> {
  ...
  ipcMain.handle('particlePreview:prepare', (_event, animation: unknown, autoplay: boolean, value?: string, armed?: boolean, loop?: boolean) => {
    const resolved = desktop.project.resolveProjectPath(workflowRoot, value);
    const preparation = desktop.particlePreview.prepareParticleAnimationPreviewApp(
      workflowRoot,
      resolved,
      toIpcPayload(animation),
      { autoplay: autoplay !== false, armed: armed === true, loop: loop === true },
    );
