...
test('product-plugin IPC keeps defaults disabled and persists only through workspace settings', async () => {
  ...
  assert.equal(listed.snapshot.find((entry: { id: string }) => entry.id === 'unlimited-map-layers')?.enabled, false)
  assert.equal(listed.snapshot.find((entry: { id: string }) => entry.id === 'game-version')?.enabled, false)
  assert.equal(listed.snapshot.find((entry: { id: string }) => entry.id === 'game-packaging')?.enabled, false)
  ...
  const layersEnabled = await handlers.get('product-plugin:set-enabled')!(null, { id: 'unlimited-map-layers', enabled: true })
  assert.equal(layersEnabled.ok, true)
  assert.equal(layersEnabled.settings['unlimited-map-layers'], true)
  const packagingEnabled = await handlers.get('product-plugin:set-enabled')!(null, { id: 'game-packaging', enabled: true })
  assert.equal(packagingEnabled.ok, true)
  assert.equal(packagingEnabled.settings['game-packaging'], true)
