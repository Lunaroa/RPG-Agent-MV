...
import AssetGridFontThumb from './AssetGridFontThumb.vue'
import AssetGridVideoThumb from './AssetGridVideoThumb.vue'
import AssetGridEffectThumb from './AssetGridEffectThumb.vue'
import AssetVideoViewer from './AssetVideoViewer.vue'
import AssetEffectViewer from './AssetEffectViewer.vue'
import ParticleAnimationPreviewFrame from './ParticleAnimationPreviewFrame.vue'
import AssetReferencesDialog from './AssetReferencesDialog.vue'
import ProjectAssetsAudioBar, { type AssetsAudioBarItem } from './ProjectAssetsAudioBar.vue'
import PluginFileFolderThumb from './editor/PluginFileFolderThumb.vue'
...
function categoryForCurrentResourcePath(nodes: ProjectAssetCategoryTreeNode[]): string | undefined {
  ...
  const visit = (items: ProjectAssetCategoryTreeNode[]) => {
    for (const node of items) {
      const directory = node.directory.replace(/\\/g, '/').replace(/^www\//, '').replace(/\/+$/, '')
      if (directory && (current === directory || current.startsWith(`${directory}/`)) && (!match || directory.length > match.length)) {
        match = { id: node.id, length: directory.length }
      }
...
const treeCoordinator = new LatestAsyncCoordinator<{ project: string }>()
...
async function ensureFolderPreview(categoryId: string) {
  ...
  try {
    ...
    if (folderPreviewGeneration === generation && projectStore.currentProject === project) {
...
const selectedResourcePath = computed(() => {
  const entry = singleSelectedFile.value
  if (!entry || !projectAssetCategoryMatchesUiDesignerResourceKind(entryCategoryId(entry), props.resourceKind)) return ''
  try {
    return normalizeUiDesignerProjectRelativeResourcePath(entryPrimaryPath(entry))
  } catch {
    return ''
  }
})
const selectedResourcePaths = computed(() => selectedFileEntries.value.flatMap((entry) => {
  if (!projectAssetCategoryMatchesUiDesignerResourceKind(entryCategoryId(entry), props.resourceKind)) return []
  try {
    const path = normalizeUiDesignerProjectRelativeResourcePath(entryPrimaryPath(entry))
    return path ? [path] : []
  } catch {
    return []
  }
...
async function loadTree(preferredCategoryId?: string) {
  ...
  const project = projectStore.currentProject
  if (!project) {
    treeCoordinator.invalidate({ project: '' })
    treeNodes.value = []
    selectedCategoryId.value = ''
    categoryEntries.value = []
    treeError.value = ''
  ...
  const token = treeCoordinator.begin({ project })
  ...
  try {
    ...
    if (treeCoordinator.isCurrent(token)) treeLoading.value = false
  }
}
...
watch(() => [route.query.category, route.query.entry], () => {
...
onMounted(() => {
  ...
  const routeCategory = !isSelectionMode.value && typeof route.query.category === 'string' ? route.query.category : undefined
...
<template>
  <div
    ...
    :class="{ 'has-preview-panel': previewPanelVisible, 'is-selection-mode': isSelectionMode }"
    :style="previewPanelVisible
      ? { '--project-assets-preview-width': `${previewPanelWidth}px` }
      : undefined"
    :data-ui-id="isSelectionMode ? 'project-assets-selection-workspace' : 'project-assets-page'"
  >
...
.project-assets-selection-actions {
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 8px 12px;
