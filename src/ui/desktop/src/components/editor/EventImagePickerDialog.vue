  ...
  <teleport to="body">
    <div v-if="visible" class="sub-overlay editor-modal-overlay" :data-editor-dialog-layer="LAYER_Z.subDialog" @mousedown.self="close">
      <section class="sub-dialog image-dialog editor-modal-shell" role="dialog" aria-modal="true" aria-labelledby="event-image-title">
        <header class="editor-modal-header"><strong id="event-image-title" class="editor-modal-title">事件图像</strong><button type="button" class="editor-modal-close" aria-label="关闭事件图像选择" title="关闭" @click="close">×</button></header>
        <div class="image-tabs editor-tab-strip" aria-label="图像类型"><button type="button" :class="{ active: tab === 'character' }" @click="tab = 'character'">角色图</button><button type="button" :class="{ active: tab === 'tile' }" @click="tab = 'tile'">图块</button></div>
        <div v-if="tab === 'character'" class="picker-grid">
          <aside>
            <input v-model="search" placeholder="搜索角色图" />
            <button type="button" :class="{ active: !draft.characterName }" @click="selectCharacter('')">(无)</button>
        ...
        <div v-else class="tile-picker">
          <nav class="editor-tab-strip" aria-label="图块页签"><button v-for="entry in tileTabs" :key="entry.label" type="button" :class="{ active: tileTab === entry.label }" :disabled="!entry.image" @click="tileTab = entry.label">{{ entry.label }}</button></nav>
          <canvas ref="tileCanvas" width="768" height="768" @click="pickTileCell" />
        </div>
        <footer class="editor-modal-footer"><span class="editor-dialog-status">{{ summary }}</span><button type="button" class="editor-btn" @click="close">取消</button><button type="button" class="editor-btn primary" @click="commit">确定</button></footer>
      </section>
    </div>
  </teleport>
</template>
...
const tileTab = ref('B');
const summary = computed(() => imageSummary(draft.value));
