    ...
    <div class="state-list">
      <template v-if="kind === 'switch'">
        <label v-for="entry in filteredSwitches" :key="entry.id" class="state-row">
          <span><b><i v-if="entry.mapReachable" class="reachable-dot" :title="t('editor.preview.mapReachable')" :aria-label="t('editor.preview.mapReachable')"></i>{{ formatId(entry.id) }}</b><em>{{ displayName(entry) }}</em></span>
...
import type { MapPreviewStateEntry } from '../../api/client';
...
function displayName(entry: MapPreviewStateEntry) {
  return entry.name.trim() || t('editor.preview.unnamed');
}
