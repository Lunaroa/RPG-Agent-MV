...
import { clampPreviewPan, previewPointerMovedBeyondClick, previewVisibleRegion } from '../../utils/mapPreviewViewport';
...
function isRuntimeEvent(value: unknown): value is MapPreviewRuntimeEvent {
  ...
  return event.kind === 'rpg-agent-map-preview'
    ...
    && ['ready', 'loading-map', 'suspended', 'state', 'fps', 'console', 'error'].includes(String(event.phase));
}
