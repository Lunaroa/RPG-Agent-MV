...
describe('editor semantic controls', () => {
  ...
  test('keeps editor modes prominent and gives drawing tools distinct icons', () => {
    ...
    assert.doesNotMatch(editorToolbarSource, /staging-actions|stagingDirty|applyStaging/);
    assert.doesNotMatch(editorViewSource, /staging-dirty|stagingDirty|applyProjectStaging|discardProjectStaging/);
