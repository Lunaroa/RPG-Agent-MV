...
const allMenuNodesVisible = (id: string) => selectedMenuNodes(id).every((node) => node.props.visible)
const allMenuNodesLocked = (id: string) => selectedMenuNodes(id).every((node) => node.locked)
...
const nodeActionLabel = (command: UiNodeActionCommand, id: string) => {
  if (command === 'toggleVisibility') return allMenuNodesVisible(id) ? t('hideNode') : t('showNode')
  if (command === 'toggleLock') return allMenuNodesLocked(id) ? t('unlockNode') : t('lockNode')
  return t(nodeActionLabels[command])
}
const nodeActionIcon = (command: UiNodeActionCommand, id: string) => {
  if (command === 'toggleVisibility') return allMenuNodesVisible(id) ? EyeOff : Eye
  if (command === 'toggleLock') return allMenuNodesLocked(id) ? Unlock : Lock
  return UI_DESIGNER_NODE_ACTION_ICONS[command]
}
const treeMenuGroups = [
  ...UI_DESIGNER_NODE_ACTION_GROUPS.slice(0, 3),
  ['expandAll', 'collapseAll'] as const,
  ...UI_DESIGNER_NODE_ACTION_GROUPS.slice(3),
]
  ...
  <section class="node-panel" data-ui-id="ui-designer-node-panel" data-testid="ui-designer-node-panel" tabindex="0" @keydown="handleKeydown">
    <div class="panel-heading">
      ...
      <span class="panel-heading-actions">
        ...
        <el-button size="small" text type="danger" :disabled="!selectedIds[0] || !nodePolicy(selectedIds[0]).allowed.delete" @click="selectedIds[0] && designer.executeNodeAction('delete', selectedIds[0])">{{ t('deleteNode') }}</el-button>
      </span>
    </div>
      ...
      <template #default="{ data }">
        ...
        <span class="node-tree-entry" :class="{ selected: selectedIds.includes(data.id), locked: document.nodes.find((node) => node.id === data.id)?.locked }" :data-node-id="data.id" :data-ui-id="`ui-designer-tree-row-${data.id}`" @mouseenter="designer.setHoveredNode(data.id)" @mouseleave="designer.setHoveredNode(undefined)" @contextmenu="designer.selectNodeActionTarget(data.id)" @dblclick.stop="emit('activateNode', data.id)">
          ...
          <span v-if="selectedIds.length < 2" class="node-row-actions">
            ...
            <el-button size="small" text type="danger" :disabled="!nodePolicy(data.id).allowed.delete" :data-ui-id="`ui-designer-tree-delete-${data.id}`" @click.stop="designer.executeNodeAction('delete', data.id)"><Trash2 /></el-button>
          </span>
        </span>
          <template #dropdown><el-dropdown-menu>
            <template v-for="(group, groupIndex) in treeMenuGroups" :key="groupIndex">
              <el-dropdown-item
                v-for="(command, commandIndex) in group"
                :key="command"
                :command="command"
                :divided="groupIndex > 0 && commandIndex === 0"
                :disabled="command !== 'expandAll' && command !== 'collapseAll' && !nodePolicy(data.id).allowed[command]"
                :data-ui-id="`ui-designer-tree-command-${data.id}-${command}`"
              >
                <component :is="command === 'expandAll' ? ChevronsDown : command === 'collapseAll' ? ChevronsUp : nodeActionIcon(command, data.id)" class="node-menu-icon" aria-hidden="true" />
                <span>{{ command === 'expandAll' ? t('expandAll') : command === 'collapseAll' ? t('collapseAll') : nodeActionLabel(command, data.id) }}</span>
              </el-dropdown-item>
            </template>
    ...
    <div class="node-types" data-ui-id="ui-designer-node-palette" data-testid="ui-designer-node-palette">
      <el-button v-for="type in paletteNodeTypes" :key="type" :data-ui-id="`ui-designer-palette-${type}`" :data-testid="`ui-designer-palette-${type}`" :aria-label="labelFor(type)" size="small" plain draggable="true" @dragstart="(event: DragEvent) => event.dataTransfer?.setData('text/ui-node-type', type)" @click="addNode(type)">
        <component :is="UI_DESIGNER_NODE_TYPE_ICONS[type]" class="node-type-icon" aria-hidden="true" />
...
.panel-heading-actions { display: inline-flex; min-width: 0; align-items: center; }
.panel-heading-actions .el-button { margin-left: 0; padding-inline: 5px; }
