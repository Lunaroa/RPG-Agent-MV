...
describe('UI Designer shared project asset workspace', () => {
  test('uses the complete manager in selection mode without the retired lightweight picker', () => {
    ...
    assert.doesNotMatch(inspector, /resource-workspace-resize-handle/)
    assert.match(inspector, /class="resource-workspace-resize-frame"/)
    assert.match(inspector, /v-for="edge in resourceWorkspaceResizeEdges"/)
    assert.match(inspector, /@pointerdown="beginResourceWorkspaceResize\(\$event, edge\)"/)
    assert.match(inspector, /\.resource-workspace-resize-edge\.ne, \.resource-workspace-resize-edge\.sw \{[^}]*cursor: nesw-resize;/)
    assert.match(inspector, /\.resource-workspace-resize-edge\.nw, \.resource-workspace-resize-edge\.se \{[^}]*cursor: nwse-resize;/)
    assert.match(inspector, /window\.addEventListener\('pointermove', moveResourceWorkspaceResize\)/)
    ...
    assert.match(workspace, /allImageDirectories: isSelectionMode\.value && props\.resourceKind === 'image'/)
    assert.match(workspace, /projectAssets\.browseTree\(project, browseOptions\.value\)/)
