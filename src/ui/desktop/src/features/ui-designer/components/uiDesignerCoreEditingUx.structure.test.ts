...
test('Inspector padding directions are visible and property expressions can be resized vertically', () => {
  ...
  const condition = compile('./UiConditionEditor.vue')
  const codePanel = compile('./UiDesignerCodePanel.vue')
  ...
  assert.match(events, /:rows="4" resizable/)
  assert.match(events, /:rows="3" resizable/)
  assert.match(condition, /:rows="3" resizable/)
  assert.match(codePanel, /:rows="18" resizable/)
...
test('canvas double click edits text and buttons in place and routes other node types to their primary editor', () => {
  ...
  assert.match(fabricCanvas, /const content = node\.type === 'button' \? normalizeUiSingleLineText\(object\.text\) : String\(object\.text \?\? ''\)[\s\S]*props\.designer\.previewNodeProperty\(node\.id, 'content', content\)/)
