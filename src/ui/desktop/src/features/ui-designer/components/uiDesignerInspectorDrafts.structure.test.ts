...
describe('UI Designer Inspector transaction drafts', () => {
  ...
  test('separates list defaults from optional track overrides and hides unsupported code entry', () => {
    const inspector = compile('UiDesignerInspector.vue')
    const field = compile('UiPropertyField.vue')
    const messages = read('../i18n.ts')
    assert.match(inspector, /type InspectorPurpose = 'layout'/)
    assert.match(inspector, /const LIST_LAYOUT_FIELDS = new Set/)
    assert.match(inspector, /layout: 'inspectorGroupListLayout'/)
    assert.match(inspector, /:allow-code="fieldAllowsCode\(field\)"/)
    assert.match(inspector, /placeholder: t\('placeholderColumnWidths'\)/)
    assert.match(inspector, /placeholder: t\('placeholderRowHeights'\)/)
    assert.match(field, /v-if="props\.allowCode \|\| props\.mode === 'code'"/)
    assert.match(field, /v-if="props\.allowCode"/)
    assert.match(field, /:placeholder="props\.placeholder \|\| '100, 200, \.\.\.'"/)
    assert.match(messages, /inspectorGroupListLayout: '列表布局'/)
    assert.match(messages, /itemWidth: '默认项宽度'/)
    assert.match(messages, /columnWidths: '列宽覆盖'/)
  })
})
