...
test('new scene surface compiles and shares the runtime scene-name guard through confirmation', () => {
  ...
  assert.match(surface, /emit\('nameEdited'\)/)
  assert.match(shell, /const newSceneNameAutomatic = ref\(true\)/)
  assert.match(shell, /if \(!newSceneNameAutomatic\.value\) return[\s\S]*newSceneDraft\.name = isBuiltInUiDesignerTemplate\(template\)/)
  assert.match(shell, /@name-edited="newSceneNameAutomatic = false"/)
